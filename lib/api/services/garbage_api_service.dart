...
class GarbageApiService {
  ...
  GarbageApiService(this._apiClient);

  /// Get all garbage vehicles with basic info
  Future<List<Vehicle>> getGarbageVehicles() async {
    try {
      final response = await _apiClient.dio.get(ApiEndpoints.getGarbageVehicles);
      ...
      throw Exception(
        'Failed to get garbage vehicles: ${response.data['message'] ?? 'Unknown error'}',
      );
  ...
  /// Get all garbage vehicles with their latest locations and institute data
  /// Returns a list of maps containing institute, vehicle, and location data
  Future<List<Map<String, dynamic>>> getGarbageVehiclesWithLocations() async {
    try {
      debugPrint('=== GARBAGE API SERVICE: Fetching vehicles with locations ===');
      debugPrint('Endpoint: ${ApiEndpoints.getGarbageVehiclesWithLocations}');
      
      final response = await _apiClient.dio.get(
        ApiEndpoints.getGarbageVehiclesWithLocations,
      );
      ...
      throw Exception(
        'Failed to get garbage vehicles with locations: ${response.data['message'] ?? 'Unknown error'}',
      );
    } on DioException catch (e) {
      ...
      throw Exception('Failed to get garbage vehicles with locations: $e');
    }
  }
  ...
  Future<Map<String, dynamic>> subscribeToVehicle(
    ...
    try {
      final response = await _apiClient.dio.post(
        ApiEndpoints.subscribeToGarbageVehicle,
        data: {
          'imei': imei,
  ...
  Future<bool> unsubscribeFromVehicle(String imei) async {
    try {
      final response = await _apiClient.dio.delete(
        ApiEndpoints.unsubscribeFromGarbageVehicle,
        data: {'imei': imei},
      );
  ...
  Future<List<Map<String, dynamic>>> getMySubscriptions() async {
    try {
      final response = await _apiClient.dio.get(
        ApiEndpoints.getMyGarbageVehicleSubscriptions,
      );
  ...
  Future<Map<String, dynamic>> updateSubscriptionLocation(
    ...
    try {
      final response = await _apiClient.dio.put(
        ApiEndpoints.updateGarbageVehicleSubscriptionLocation
