...
class PublicVehicleIndexScreen extends StatefulWidget {
...
class _PublicVehicleIndexScreenState extends State<PublicVehicleIndexScreen> {
  ...
  Future<void> _loadSubscriptions() async {
    try {
      ...
      final subscriptions = await publicVehicleApiService.getMySubscriptions();
      setState(() {
  ...
  Future<void> _getCurrentLocation() async {
    ...
    try {
      ...
      // Load public vehicles after getting location
      _loadPublicVehicles();
      _loadSubscriptions();
    } catch (e) {
      setState(() {
  ...
  Future<void> _loadPublicVehicles() async {
    setState(() {
    ...
    try {
      ...
      setState(() {
        _publicVehiclesData = vehiclesData;
        _isLoadingVehicles = false;
      });

      debugPrint('=== UPDATING MARKERS ===');
      debugPrint('Vehicles data count: ${_publicVehiclesData.length}');
      await _updateMarkers();
    } catch (e) {
      debugPrint('=== ERROR LOADING PUBLIC VEHICLES ===');
      debugPrint('Error: $e');
      ...
      Get.snackbar(
        'Error',
        'Failed to load public vehicles: ${e.toString()}',
        backgroundColor: Colors.red,
        colorText: Colors.white,
        snackPosition: SnackPosition.TOP,
      );
        ...
        newMarkers.add(
          Marker(
            ...
            infoWindow: InfoWindow(
              title: vehicle.name ?? vehicle.vehicleNo ?? 'Public Vehicle',
  ...
  Future<void> _handleSetNotify(Vehicle vehicle) async {
    ...
    try {
      ...
      if (isSubscribed) {
        // Unsubscribe
        await publicVehicleApiService.unsubscribeFromVehicle(vehicle.imei);
        setState(() {
        ...
        // Subscribe
        await publicVehicleApiService.subscribeToVehicle(
          vehicle.imei,
